...
//
// Purpose: A modified Crossbow to be a camera
//
//=============================================================================//
...
void CCameraBolt::BoltTouch( CBaseEntity *pOther )
{
	...
	if ( pOther->m_takedamage != DAMAGE_NO )
	{
		...
		//!!!HACKHACK - specific hack for ep2_outland_10 to allow camera bolts to pass through her bounding box when she's crouched in front of the player
		// (the player thinks they have clear line of sight because Alyx is crouching, but her BBOx is still full-height and blocks camera bolts.
		...
		{
			CTakeDamageInfo	dmgInfo( this, GetOwnerEntity(), sk_plr_dmg_camera.GetFloat(), DMG_NEVERGIB );
			...
			if ( pPlayer )
			{
				gamestats->Event_WeaponHit( pPlayer, true, "weapon_camera", dmgInfo );
			}

		}
		else
		{
			CTakeDamageInfo	dmgInfo( this, GetOwnerEntity(), sk_plr_dmg_camera.GetFloat(), DMG_BULLET | DMG_NEVERGIB );
			CalculateMeleeDamageForce( &dmgInfo, vecNormalizedVel, tr.endpos, 0.7f );
			dmgInfo.SetDamagePosition( tr.endpos );
			pOther->DispatchTraceAttack( dmgInfo, vecNormalizedVel, &tr );
		}
...
#define	CAMERA_GLOW_SPRITE	"sprites/light_glow02_noz.vmt"
#define	CAMERA_GLOW_SPRITE2	"sprites/blueflare1.vmt"
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponCamera::CheckZoomToggle( void )
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponCamera::ItemBusyFrame( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponCamera::ToggleZoom( void )
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponCamera::CreateChargerEffects( void )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	...
	m_hChargerSprite = CSprite::SpriteCreate( CAMERA_GLOW_SPRITE, GetAbsOrigin(), false );
...
// Input  : skinNum - 
//-----------------------------------------------------------------------------
void CWeaponCamera::SetSkin( int skinNum )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponCamera::DoLoadEffect( void )
{
	SetSkin( BOLT_SKIN_GLOW );
	...
	DispatchEffect( "CameraLoad", data );

	CSprite *pBlast = CSprite::SpriteCreate( CAMERA_GLOW_SPRITE2, GetAbsOrigin(), false );
...
// Input  : state - 
//-----------------------------------------------------------------------------
void CWeaponCamera::SetChargerState( ChargerState_t state )
{
...
//-----------------------------------------------------------------------------
bool CWeaponCamera::SendWeaponAnim( int iActivity )
{
...
//-----------------------------------------------------------------------------
void CWeaponCamera::StopEffects( void )
{
	// Stop zooming
	if ( m_bInZoom )
	{
